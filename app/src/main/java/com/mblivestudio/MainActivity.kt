...
class MainActivity : Activity(), EngineCallback, SurfaceHolder.Callback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnMicToggle.setOnClickListener {
            ...
            streamEngine.muteAudio(isAudioMuted)
            btnMicToggle.setImageResource(if (isAudioMuted) R.drawable.ic_mic_off else R.drawable.ic_mic_on)
        }
        ...
        ivProfilePhoto.setOnClickListener { if (GoogleSignIn.getLastSignedInAccount(this) == null) { startActivityForResult(googleSignInClient.signInIntent, SIGN_IN_REQUEST) } else { Toast.makeText(this, "Logged in as: $connectedAccountEmail", Toast.LENGTH_SHORT).show() } }

        btnGoLive.setOnClickListener {
            if (streamEngine.rtmpCamera.isStreaming) { AlertDialog.Builder(this).setTitle("Stop Live Stream?").setMessage("This will end your YouTube broadcast permanently.").setPositiveButton("End Stream") { _, _ -> stopLiveStream() }.setNegativeButton("Cancel", null).show(); return@setOnClickListener }
            if (!streamEngine.rtmpCamera.isOnPreview) { tryStartCameraPreview(); Toast.makeText(this, "Camera starting, try LIVE again in a moment.", Toast.LENGTH_SHORT).show(); return@setOnClickListener }
    ...
    private fun stopLiveStream() {
        ...
        Thread {
            ...
            runOnUiThread {
                try { streamEngine.stopPreview() } catch (e: Exception) {}
        ...
        Thread {
            try {
                ...
                runOnUiThread {
                    ...
                    try { streamEngine.startStream(finalUrl); if (liveChatId != null) startChatPolling(liveChatId) } catch (e: Exception) { btnGoLive.text = "LIVE"; btnGoLive.isEnabled = true; Toast.makeText(this@MainActivity, "Stream Error: Failed to start.", Toast.LENGTH_LONG).show() }
                }
    ...
    override fun surfaceDestroyed(holder: SurfaceHolder) { 
        surfaceReady = false
        if (streamEngine.rtmpCamera.isStreaming) streamEngine.stopStream()
        if (streamEngine.rtmpCamera.isOnPreview) streamEngine.stopPreview() 
    }
    ...
    private fun applyCameraLayout(rect: FloatArray) { 
        Toast.makeText(this, "Camera layouts incoming in Phase 2", Toast.LENGTH_SHORT).show() 
    }
    ...
    private fun sendSyntheticZoomEvent(action: Int, pointerDistance: Float, delta: Float) { 
        ...
        coords[0].x = 0f; coords[0].y = 0f; coords[1].x = pointerDistance; coords[1].y = 0f

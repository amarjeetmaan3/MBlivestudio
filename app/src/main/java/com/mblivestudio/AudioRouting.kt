    ...
    audioDeviceCallback = object : AudioDeviceCallback() {
        ...
        override fun onAudioDevicesRemoved(removedDevices: Array<out AudioDeviceInfo>) {
            if (removedDevices.any { it.type == AudioDeviceInfo.TYPE_BLUETOOTH_SCO || (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S && it.type == AudioDeviceInfo.TYPE_BLE_HEADSET) } && isBluetoothMicActive) {
                isBluetoothMicActive = false; bluetoothCommunicationDevice = null; clearBluetoothRoute(); activity.findViewById<ImageButton>(R.id.btnBluetoothMic).clearColorFilter(); restartCameraForAudioChange(250)
            }
            updateDetectedMicRoute(false)
        }
    }
    ...
    if (!isBluetoothMicActive) {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                ...
                val receiver = object : android.content.BroadcastReceiver() {
                    override fun onReceive(context: Context?, intent: Intent?) {
                        when (intent?.getIntExtra(AudioManager.EXTRA_SCO_AUDIO_STATE, -1)) {
                            AudioManager.SCO_AUDIO_STATE_CONNECTED -> { scoConnectTimeoutHandler.removeCallbacksAndMessages(null); try { activity.unregisterReceiver(this) } catch (e: Exception) {}; scoStateReceiver = null; restartCameraForAudioChange(150) }
                            AudioManager.SCO_AUDIO_STATE_DISCONNECTED -> { scoConnectTimeoutHandler.removeCallbacksAndMessages(null); try { activity.unregisterReceiver(this) } catch (e: Exception) {}; scoStateReceiver = null; isBluetoothMicActive = false; bluetoothCommunicationDevice = null; updateDetectedMicRoute(false); activity.findViewById<ImageButton>(R.id.btnBluetoothMic).clearColorFilter() }
                        }
                    }
                }
                ...
                scoConnectTimeoutHandler.postDelayed({ scoStateReceiver?.let { try { activity.unregisterReceiver(it) } catch (e: Exception) {} }; scoStateReceiver = null; isBluetoothMicActive = false; bluetoothCommunicationDevice = null; try { audioManager.stopBluetoothSco(); audioManager.isBluetoothScoOn = false; audioManager.mode = AudioManager.MODE_NORMAL } catch (e: Exception) {}; updateDetectedMicRoute(false); activity.findViewById<ImageButton>(R.id.btnBluetoothMic).clearColorFilter() }, 8000)
            }
            ...
            Toast.makeText(activity, "Bluetooth mic selected", Toast.LENGTH_SHORT).show(); restartCameraForAudioChange(300)
        } catch (e: Exception) { e.printStackTrace() }
    } else {
        clearBluetoothRoute(); isBluetoothMicActive = false; bluetoothCommunicationDevice = null; button.clearColorFilter()
        Toast.makeText(activity, "Bluetooth mic off", Toast.LENGTH_SHORT).show(); restartCameraForAudioChange(250)
    }
}
...
internal fun MainActivity.clearBluetoothRoute() {
    val activity = this
    scoStateReceiver?.let { try { activity.unregisterReceiver(it) } catch (e: Exception) {} }; scoStateReceiver = null; scoConnectTimeoutHandler.removeCallbacksAndMessages(null)
    try {
